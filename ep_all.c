...
void EP_EventScript ( edict_t *self, char *scriptname ) // assault virgin11beta
{
	FILE *infile; // assault virgin11beta
	char readfile[64];
	char buffer[128];
	int trys = 0;
	char dir[32];
	...
	// Open script file assault virgin alpha
	...
	Com_sprintf (readfile, sizeof(readfile), "%s"DIR_SLASH"%s",dir,scriptname);
...
#if CHIEFCHAT
	gi.bprintf(PRINT_HIGH,"\nLoading Chief Script \"%s\" into %s\'s Memory!\n", readfile, self->classname);
#endif
	
	self->count = trys = 0;
	while (!feof(infile) && trys < 16 ) // read file.
	{
		getline(self,infile,buffer); // search the document.
		trys ++;
	}
	self->count--;

	self->option = self->count;
