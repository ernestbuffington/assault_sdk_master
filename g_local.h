...
#define	SCOREBOARD			2
#define	SPECTATORS			3
#define SCORE_REJOIN		4
#define SCORE_MAP_VOTE		5
...
// ASSAULT2 for_everyone
#define for_everyone(GUY,NUM) for(NUM=1;NUM<=maxclients->value;NUM++)if((GUY=&g_edicts[NUM]) && GUY->inuse)

// the "gameversion" client command will print this plus compile date
#define	GAMEVERSION	"Team Assault v.024"
#define CHIEF 0
#define CHIEFGFX 1
#define CHIEFCHAT 0
...
// RAFAEL // virgin16 and virgin17 and virgin23
typedef enum
{
	CLIP_NONE,
	CLIP_PISTOL,
...
// level.Chief
	char	*runts_key1_target;
	char	*runts_key2_target;
	char	*runts_key3_target;
	char	*thugs_key1_target;
	char	*thugs_key2_target;
	char	*thugs_key3_target;
} level_locals_t;
...
#define MOD_SPAZ			3 // assault2 virgin16
#define MOD_MACHINEGUN		4
#define MOD_PUMP			5 // assault2 virgin17
...
#define MOD_AK47				34  /* assault2 virgin23 */
#define MOD_EXPLODINGKEY		35
...
// Chief
//	int shotgunfrags; // assault2 virgin18
...
struct edict_s //all this stuff can be accessed by Kprad.
{
	entity_state_t	s;
	...
	int			count; // assault virgin11 hijack for dog tag.
...
//  Chief
	char		gesture[16][64]; // assault virgin11beta
};
...
#define UPDATERATE(e) if (specrate) {\
	char buf[32];\
	sprintf(buf,"hand %d\nhand %d\n",e->client->pers.hand^1,e->client->pers.hand);\
	gi.WriteByte(13);\
	gi.WriteString(buf);\
	gi.unicast(e, true);\
	e->client->resp.lastrate=0;\
}

#define KICKENT(ent,mess) {ent->kickmess=mess;ent->kickdelay=2;}

#define RESPAWN_ASSAULT(dude) { dude->client->pers.spectator = PLAYING; \
	dude->flags &= ~FL_GODMODE; dude->health = 0; \
	meansOfDeath = MOD_RESTART;	dude->solid = SOLID_NOT; \
	player_die (dude, dude, dude, 1, vec3_origin, 0, 0); \
	PutClientInServer(dude); dude->client->resp.is_spawn = true; }
...
#define WINNER 101
#if CHIEFGFX
#define PIC_KPLLOGO "logo"
#endif

int cutscenetime;
int AcceptableThugSpawnerStyle;
int AcceptableRuntSpawnerStyle;
void Cmd_AssaultStatus();
void DropKeys(edict_t *self); //Assault2
void AssaultKeyThink (edict_t *ent);//assault2
void Cmd_Dog_f (edict_t *self, char *secondword);
void DogCheckForEnemies( edict_t *self );
void Dog_Body_Animate( edict_t *ent );
void dog_respawn(edict_t *self);

extern int assault_self_rocket_percent_dmg;
extern int assault_keytargets;
//extern int assault_customweaponfrags;
extern int assault_replaymaps;
extern int assault_status;
extern int assault_auto_switch_teams;
extern int CustBlakjakDmg;
extern int CustPistolDmg;
extern int CustShotgunDmg;
extern int CustTommyDmg;
extern int CustHMGDmg;
extern int CustGrenadeDmg;
extern int CustRocketDmg;
extern int CustFlameDmg;


// seperate files.. ai_assault.c
void Die_Body_Animate( edict_t *ent );
qboolean CheckScripts(edict_t *self); // thug
...
#define CHECKDAMAGE else if(strstr(self->gesture[n],"_dmg"))   { \
			char damage[8]; GetValue(damage,self->gesture[n]); \
			self->dmg = atoi(damage); \
			if(self->count-- < 0) return false;	goto virgin13; }

#define CHECKTARGET	else if(strstr(self->gesture[n],"target")) { \
			strcpy(self->skins,self->gesture[n]); \
			if(self->count-- < 0) return false;	goto virgin13; } CHECKDAMAGE

#define CHECKPLAYSOUND virgin13: n = self->option - self->count; \
			if( strstr(self->gesture[n],"play")) \
			{ int	q=0; char buffer[64]; \
			while(self->gesture[n][q] && q < 64) \
			{	if(q>=5) buffer[q-5] = self->gesture[n][q];	q++; } \
			q-=5;	buffer[q] = '\0'; \
			gi.sound( self, CHAN_VOICE, gi.soundindex( buffer ), 1.0, 1, 0 ); \
			if(self->count-- < 0) return false;	goto virgin13; } CHECKTARGET
