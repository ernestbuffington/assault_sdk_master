...
void Touch_Multi (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	if(other->client)
	{
		...
		if (self->style && other->client->pers.team != self->style)
		{
			...
			else if(self->target && (self->spawnflags & 64)) 
				gi.centerprintf (other, "This %s cant be used by your team.",self->target);
			else if (self->spawnflags & 64)	gi.centerprintf (other, "This is not available to your team.");
...
{
	if(ent->type) ent->noise_index = gi.soundindex (ent->type); //assault2
	...
	else if (ent->sounds == 1)//assault2
		ent->noise_index = gi.soundindex ("misc/menu1.wav");
	else if (ent->sounds == 2)
		ent->noise_index = gi.soundindex ("misc/menu2.wav");
	else if (ent->sounds == 3)
		ent->noise_index = gi.soundindex ("misc/menu3.wav");
	else if (ent->sounds == 7)
		ent->noise_index = gi.soundindex ("world/pawnomatic/menubuzz.wav");

	if (!ent->wait)	ent->wait = 0.2;
	if (!ent->option) ent->option = 0; //assault2 for message broadcasting mode.
