...
{
	fire_blaster (self, start, dir, damage, speed, effect, false, self);
...
void walking_cast_start_go (edict_t *self)
{
	...
	{
		if (!Q_stricmp(self->classname, "cast_whore"))
		{
//			strcpy(self->name , "Whore");
			gi.dprintf( "\nFriendly %s is gets default name.\n  All friendly characters MUST have a name.\n\n", self->classname);// , self->name);
		}
		else
gi.dprintf( "\n** Warning ** Friendly %s is without a NAME!\n  All friendly characters MUST have a name.\n\n", self->classname);
	}
