...
void target_explosion_explode (edict_t *self)
{
	...
	if(!self->fxdensity) self->fxdensity = 50;

	if(!strcmp(self->targetname, "dropc"))
		{
			self->fxdensity = 90;
...
////////////////////////////////////////////// ASSAULT2
	if(!strcmp(self->targetname, "nuke"))
		T_RadiusDamage (self, self->activator, self->dmg, NULL, self->dmg+40, MOD_NUKE); // FRANK
     else if (strstr(level.mapname, "assault"))
		T_RadiusDamage (self, self->activator, self->dmg, NULL, self->dmg+40, MOD_CANNON); // FRANK
///////////////////////////////////////////// END ASSAULT2
	else T_RadiusDamage (self, self->activator, self->dmg, NULL, self->dmg+40, MOD_EXPLOSIVE);
	save = self->delay;
	self->delay = 0;
...
void use_target_spawner (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	if(!Q_stricmp(self->target,"item_watch")) //assault_jungle drops bombs like this.
	{	
		if(self->style == 3)
		{
		vec3_t start;
		vec3_t aimdir;
		VectorSet( start, self->s.origin[0], self->s.origin[1], self->s.origin[2] ); // set start origins.
		VectorSet(aimdir, self->s.angles[PITCH], self->s.angles[YAW], self->s.angles[ROLL]);
		if(!self->speed)
			self->speed = 100;
		if(!self->dmg)
			self->dmg = 100;
		if(!self->wait) // detonation timer
			self->wait = 2.0;
		fire_grenade (self, start, aimdir, self->dmg, self->speed, self->wait, (self->dmg)*1.75);
		//gi.bprintf( PRINT_HIGH, "%s\n", self->target);// assault 2
		return;
		}
	}

	if (!Q_stricmp (ent->classname, "key_key1")||
		!Q_stricmp (ent->classname, "key_key2")||
		!Q_stricmp (ent->classname, "key_key3")		
	   ) // assault2
	{
		//gi.bprintf(PRINT_HIGH,"\n%s was respawned\n", ent->classname);

		ent->style = self->style;
		if(self->wait) ent->wait = self->wait;
		else ent->wait = 15.0;

		if(self->option) ent->option = self->option;
		if(self->order) ent->order = self->order;

		gi.bprintf (PRINT_HIGH,"\nReplacing %s\'s %s!\n", team_names[ent->style] , ent->classname);
	}

	if( strstr (self->target, "cast_")) // assault2
	{
		//copy everything you've read about regardless.
		ent->style = self->style;
		ent->cast_group = self->cast_group;
		ent->onarampage = self->onarampage;
		ent->skin = self->skin;
		ent->moral = self->moral;
...
//		if(!self->cast_group || (self->cast_group < 2 && !self->name)) ent->cast_group = 2;
		if(self->name)       ent->name  = self->name;
		if(self->dmg)        ent->dmg   = self->dmg;
		if(self->moral)      ent->moral = self->moral;
		if(self->style)      ent->style = self->style;
		if(self->skin)       ent->skin  = self->skin;
		if(self->cast_group) ent->cast_group = self->cast_group;
		if(self->onarampage) ent->onarampage = self->onarampage;
		if(self->profanity_level) ent->profanity_level = self->profanity_level;
		if(self->art_skins) ent->art_skins = self->art_skins;
		if(self->combattarget) ent->target = self->combattarget;
		if(self->deathtarget) ent->targetname = self->deathtarget;
	}
//	if(!strcmp(self->targetname, "bomb"))
//		gi.bprintf( PRINT_HIGH, "%s\n", self->targetname );// assault 2
...
void target_laser_think (edict_t *self)
{
	...
	while(1)
	{
		...
		if (
			...
			&& (self->style && tr.ent->client->pers.team == self->style) // virgin19
		   )
...
{
	if (!self->activator)		self->activator = self;
