...
#include "voice_bitch.h"
	...
	if( (strstr(self->classname, "cast_"))
&& ( self->timestamp < (level.time - 1) ) )// assault virgin11beta
	{
		self->timestamp = level.time;
		if(CheckScripts(self)) return;
	} // END NEW CODE
...
void AI_EndAttack(edict_t *self)
{
	...
	{
		//snatch this is where the dog stops walking.
		if (!Q_stricmp(self->classname, "cast_dog")) // BEGIN NEW CODE
		{
			if(rand()%2)
				self->cast_info.currentmove = self->cast_info.move_avoid_walk;
			else
		...
		self->cast_info.currentmove = self->cast_info.move_stand; // EXISTING CODE
...
{
//	if (!Q_stricmp(self->classname, "cast_dog")) 
	{	
		self->count = self->s.origin[2]; // compare original death z value later. 
		self->timestamp = level.time; // delay
		Die_Body_Animate(self);
	}
...
//	if (!(self->svflags & SVF_DEADMONSTER))
	...
	{
		if (Q_stricmp(self->classname, "cast_dog") && !deathmatch->value) // assalt virgin11beta
		{
			SpawnBloodPool (self); // assault vigin11
