...
//
	...
	{
////////////// ASSAULT2 Hogie wants options here to specify who message is for, Frank.
		edict_t     *other;
		int j;

		if(ent->spawnflags & 8) // MSG_NOTME // send message to activator if he cant use this trigger
		{
			if (activator->client->pers.team != ent->style)
				gi.centerprintf (activator, "%s" , ent->message);
		}

		else if(ent->spawnflags & 16) //full broadcast
		{
			for (j = 1; j <= game.maxclients; j++)
			{
				...
				gi.centerprintf (other, "%s" , ent->message);
			}
		}
		else if(ent->spawnflags & 32) //same team only
		{
			for (j = 1; j <= game.maxclients; j++)
			{
				other = &g_edicts[j];
				...
				if (!(other->client))
					continue;
				if (other->client->pers.team == ent->style)
					gi.centerprintf (other, "%s" , ent->message);
			}
		}

		else if(activator->client) 
		gi.centerprintf (activator, "%s" , ent->message); //sole activator

		if(ent->spawnflags & 64) //opposite team only
		{
			for (j = 1; j <= game.maxclients; j++)
			{
				other = &g_edicts[j];
				...
				if (other->client->pers.team == ent->style)	continue;
				if(ent->guard_target)
				gi.centerprintf (other, "%s" , ent->guard_target);
				else
				gi.centerprintf (other, "Mapper forgot a \"guard_target\" message");
			}
		}
...
//
	if (ent->target)
	{
		...
		while ((t = G_Find (t, FOFS(targetname), ent->target)))
		{
			...
			if (!Q_stricmp (t->classname, "target_blaster")) // assault2
			{
				//gi.bprintf (PRINT_HIGH, "%s aiming....\n",t->classname);
				if(ent->moral) t->moral = ent->moral;
				if(ent->order) t->order = ent->order;
			}
			...
			//	t->activate_flags |= ACTIVATE_AND_OPEN;	
				...
				t->activate_flags |= ACTIVATE_AND_OPEN;
			// JOSEPH 23-APR-99
			else if (!Q_stricmp (t->classname, "func_door") ) t->activate_flags |= ACTIVATE_AND_OPEN;

			else if (!Q_stricmp (t->classname, "func_turretbreach")) t->activate_flags |= ACTIVATE_GENERAL;			
