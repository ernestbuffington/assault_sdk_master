...
void Start_Pub () // Starts a pub
{
	...
	if (strstr(level.mapname, "assault_jungle")) //assault2
		gi.WriteString("play nam/chatter.wav");
	else if (strstr(level.mapname, "assault_forts"))
		gi.WriteString("play nam/f17flyby.wav");
	else if (strstr(level.mapname, "assault_passage"))
		gi.WriteString("play funcs/trainpass.wav");
	else if (strstr(level.mapname, "assault_tower"))
		gi.WriteString("play world/airtram.wav");
	else if (strstr(level.mapname, "assault_ty"))
		gi.WriteString("play funcs/subwayout.wav");
	else if (strstr(level.mapname, "assault_ship"))
		gi.WriteString("play world/foghorn.wav");
	else 
	{
		static int pick;
		pick=(pick+1)%4;
		...
		{
		default:
		case 0:	gi.WriteString("play funcs/subwayout.wav"); break;
		case 1:	gi.WriteString("play world/airtram.wav"); break;
		case 2:	gi.WriteString("play world/foghorn.wav"); break;
		case 3:	gi.WriteString("play funcs/trainpass.wav"); break;
		}
	}
	...
	{ //assault2
		edict_t *sp = NULL;
		edict_t *theworldspawn = NULL; // there has to be one of these so use it to trigger any start_game

		while ((theworldspawn = G_Find (theworldspawn, FOFS(classname), "worldspawn"))!= NULL)
		{
			continue;
		}
		while ((sp = G_Find (sp, FOFS(classname), "func_turretbreach")) != NULL)
		{
			if(!sp->option) sp->option = 0;
			sp->s.angles[YAW] = sp->option;
		}
		while ((sp = G_Find (sp, FOFS(classname), "func_turretbase")) != NULL)
		{
			if(!sp->option) sp->option = 0;
			sp->s.angles[YAW] = sp->option;
		}
		while ((sp = G_Find (sp, FOFS(classname), "func_turretdriver")) != NULL)
		{
			if(!sp->option) sp->option = 0;
			sp->s.angles[YAW] = sp->option;
		}	
		while ((sp = G_Find (sp, FOFS(targetname), "start_game")) != NULL)
		{
			if(theworldspawn)
			{
				theworldspawn->target = G_CopyString( "start_game" );
				G_UseTargets (theworldspawn, theworldspawn); 
			}
		}	
	}
}
...
void CheckStartPub () // 35 second countdown before server starts
{
	...
	if(level.framenum == 330) //curse
	{
		int i;
		edict_t *dude;
		for_everyone(dude,i) { }

		if(dude && dude->client && !Q_stricmp( level.mapname, "team_bel" ) )
		{
			char word[3] = "1";
			Cmd_Dog_f(dude, word);	strcpy(word,"2");	Cmd_Dog_f(dude,word);
		}
	}

	if((timing == 4)&&(level.framenum == 180)&&(level.power_cubes != 101)) //assault2
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play world/cypress4.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 3) && (level.framenum == 270) && (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play nam/honey.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 2)&&(level.framenum == 260)&& (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play cut_scenes/kpcut5.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 1)&&(level.framenum == 250)&& (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play cut_scenes/sycut.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 0)&&(level.framenum == 105)&& (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play cut_scenes/cut_pv1.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 5)&&(level.framenum == 90)&& (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play cut_scenes/kpcut7.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 6)&&(level.framenum == 275)&& (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play scenaric/idiot.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 7)&&(level.framenum == 295)&& (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play scenaric/killedtheboss.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 8)&&(level.framenum == 310)&& (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play actors/windowtoss.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 9)&&(level.framenum == 295)&& (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play actors/bum/swat.wav");//actors\female\profanity\level3
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 10)&&(level.framenum == 330)&& (level.power_cubes != 101))//world/pawnomatic/funny3.wav
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play actors/female/profanity/level3/f_cuss3-1.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	else if((timing == 11)&&(level.framenum == 330)&& (level.power_cubes != 101))
	{
		gi.WriteByte( svc_stufftext );
		gi.WriteString("play world/pawnomatic/funny3.wav");
		gi.multicast (vec3_origin, MULTICAST_ALL);
		level.power_cubes = 1;
	}
	if(level.power_cubes == 1)
	{
		timing = (timing+1)%12;
		level.power_cubes = 101;
	}

	if ((level.framenum % 10 == 0 ) && (level.framenum > 299))
		gi.bprintf(PRINT_HIGH,"Assault in %d seconds!\n", (340 - (level.framenum )) / 10);
}
