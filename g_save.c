...
// Papa 10.6.99	
	...
	if (i != OK)
	{
		if (i == FILE_OPEN_ERROR)	gi.dprintf("Assault.ini file not opened!\n");
		else
			gi.dprintf("Error opening Comp ini file!\n");
	}
	else
		gi.dprintf("Processed Assault.ini file!\n");
...
// Chief 3.4.2001 Assault2
	AcceptableThugSpawnerStyle = 1;
	AcceptableRuntSpawnerStyle = 2;

	if(MapReplay)	gi.bprintf(PRINT_HIGH, "Initialized MapReplay to True!\n");
	if(assault_status) Cmd_AssaultStatus();
