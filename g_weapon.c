...
void plasma_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (other->takedamage)
	{
		T_Damage (other, ent, ent->owner, ent->velocity, ent->s.origin, plane->normal, ent->dmg, 0, 0, MOD_EXPLODINGKEY);
	}
	
	T_RadiusDamage(ent, ent->owner, ent->radius_dmg, other, ent->dmg_radius, MOD_EXPLODINGKEY);
...
//	float r, u;
	...
	org[2] += 64.5;
				//source, scale, buffer?, target.
	VectorMA	( org, -5.5, xup, org );

	VectorCopy( org, start );
	
	VectorMA( start, 8, xup, start ); //8
...
*/
/*QUAKED target_blaster (1 0 0) (-8 -8 -8) (8 8 8) CHIEFS_MOVEIT AIM_CLIENTYAW AIM_CLIENTPITCH CHIEFS_DEBUG MUZZLE_FIREBALL MUZZLE_LRGSPARKS MUZZLE_FIRENSPARKS MUZZLE_BFGLASER
...
CHIEFS_MOVEIT will move the blaster to the activator's (path_corner's) location.
...
MUZZLE_FIREBALL will produce a fireball behind the emerging projectile.
...
void fire_blaster (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper, edict_t *activator)
{
	edict_t	*bolt = NULL;
	trace_t	tr;

	if(activator && activator->client && (self->option != 0) ) activator->bikestate = self->option;


	if(self->firetype) //assault2
	{
		switch(self->firetype)
		{
		default:
		case 7:
			fire_rocket (activator, start, dir, damage, speed, damage+20, damage);
			break;
...
//	grenade->s.renderfx  |= RF_REFL_MAP;
			break;
		case 6:
			if(!self->delay) self->delay = 2.0;
			fire_grenade (activator, start, dir, damage, speed, self->delay, damage+20);
			//	fire_bullet (ent, start, forward, damage, kick, 0, 0, MOD_BARMACHINEGUN);
			break;
		case 5:
			fire_bullet (activator, start, dir, damage, 50, 0, 0, MOD_BARMACHINEGUN);
			break;
		case 4:
			fire_bullet (activator, start, dir, damage, 50, 0, 0, MOD_MACHINEGUN);
			break;
//		case 8:
//			fire_target_flamethrower(self, start, dir, damage, 0, MOD_FLAMETHROWER); //fixme assault2
//			//fire_flamethrower (self, start, dir, damage, 10, MOD_FLAMETHROWER); //fixme assault2
//			return;//break;
		}

		if(self->spawnflags & 128)	
		{
			fire_bfg (self, start, dir, damage, speed, damage+50);//bfg_think(self); //assault2 works fine.
			return;
		}
		...
		{   
			...
			VectorClear(right);
			vec[2] = 1;
			VectorSet(offset, self->count-10, 0, self->viewheight-10);
			VectorCopy (offset, _distance);
			_distance[1] = 0;
			G_ProjectSource (self->s.origin, _distance, dir, right, start);

			gi.WriteByte (svc_temp_entity);

			if(self->spawnflags & 16)	gi.WriteByte (TE_EXPLOSION1B); //Fireball Only.
			else if(self->spawnflags & 32) gi.WriteByte (TE_EXPLOSION1C); //Sparks Only.
			else /*if (self->spawnflags & 64)*/ gi.WriteByte (TE_EXPLOSION1);// //big and sparky with fireball
			
			gi.WritePosition (start);
			gi.WriteDir(vec);
			gi.WriteByte( (int)(self->dmg) );
			gi.WriteByte (self->spawnflags);
			gi.multicast (start, MULTICAST_PVS);

			/* //assault 2 slowmotion effect
				VectorNormalize (dir);
				gi.WriteByte (svc_temp_entity);
				gi.WriteByte (TE_SFXFIREGO);
				gi.WritePosition (start);
				...
				gi.multicast (start, MULTICAST_PVS);
			*/
		//	T_RadiusDamage (ent, shooter, ent->dmg, NULL, ent->dmg+40, MOD_EXPLOSIVE);
			T_RadiusDamage (self, self, self->dmg, NULL, 40, MOD_EXPLOSIVE);
		}
	...
	VectorClear (bolt->maxs);
			...
			bolt->s.modelindex = gi.modelindex ("models/weapons/shella/tris.md2"); //assault2
			bolt->s.sound = gi.soundindex ("weapons/rocket_launcher/rl_rocket.wav");
			bolt->touch = rocket_touch;
			bolt->nextthink = level.time + 8000/speed;
			bolt->think = G_FreeEdict;
			bolt->radius_dmg = damage;
			bolt->dmg_radius = damage+50;
			bolt->classname = "rocket";
			bolt->die = rocket_die;

	bolt->health = 5; //assault2;
	bolt->takedamage = DAMAGE_YES;//assault2

	bolt->owner = self;

//	bolt->nextthink = level.time + 2;//assault2
//	bolt->think = G_FreeEdict; //assault2
	bolt->dmg = damage;
	//bolt->classname = "bolt";//assault2
