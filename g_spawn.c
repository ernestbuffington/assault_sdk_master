...
void SP_weapon_msg90 (edict_t *self); //assault2
void SP_weapon_mortar (edict_t *self); //assault2
...
spawn_t	spawns[] = {
	...
	{"func_turretdriver", SP_func_turretdriver},
	{"func_turretbase", SP_func_turretbase},
	{"func_turretbreach", SP_func_turretbreach},
...
// weapon mods
	...
	{"weapon_msg90", SP_weapon_msg90}, //assault2
	{"weapon_mortar", SP_weapon_mortar}, //assault2
	{NULL, NULL}
};
...
void ED_CallSpawn (edict_t *ent)
{
	...
	if ( !Q_stricmp( ent->classname, "weapon_msg90") ) // assault2
	{
		if(!ent->option) ent->option = 1; // reorient the model?
		switch(ent->option)
		{
			default:
			case 1:		break; // do nothing.
			case 2:		ent->s.angles[PITCH] = 240;
						ent->s.angles[YAW] = 90;
...
void CountLevelKeys(void) // Assault2 bug fix for when the game is ruined cuz a key has gone missing
{
	edict_t	*key=NULL;
	edict_t *sp;

	int thugkeyone, thugkeytwo, thugkeythree, runtkeyone, runtkeytwo, runtkeythree;

	level.runts_key1_target = NULL;
	level.runts_key2_target = NULL;
	level.runts_key3_target = NULL;
	level.thugs_key1_target = NULL;
	level.thugs_key2_target = NULL;
	level.thugs_key3_target = NULL;

	thugkeyone=thugkeytwo=thugkeythree=runtkeyone=runtkeytwo=runtkeythree=level.power_cubes=0;

	while((key = G_Find (key, FOFS(classname), "key_key1")) != NULL) 
	{
		if(!key->style) key->style = 2;

//		if(key->style > 2 || key->style < 1) key->style = 2;
		if(key->style == 1) 
		{ 

			if(key->target) { level.thugs_key1_target = G_CopyString(key->target);  key->target=NULL; }
			sp = G_Spawn();
			sp->classname = G_CopyString( "target_spawner" ); 
			//VectorSet( sp->s.origin, 580, -1830, -170 );
			VectorCopy( key->s.origin, sp->s.origin );
			sp->s.origin[2] += 10;
			sp->style = key->style;
			sp->target = G_CopyString( "key_key1" );
			sp->targetname = G_CopyString( "thugs_key1" );
			if(key->wait)	sp->wait = key->wait;
			if(key->option) sp->option = key->option;
			if(key->order)  sp->order = key->order;
			ED_CallSpawn( sp );
		...
		{ 
			...
			if(key->target) { level.runts_key1_target = G_CopyString(key->target); key->target=NULL; }
			sp = G_Spawn();
			sp->classname = G_CopyString( "target_spawner" ); 
			//VectorSet( sp->s.origin, 580, -1830, -170 );
			VectorCopy( key->s.origin, sp->s.origin );
			...
			sp->style = key->style;
			sp->target = G_CopyString( "key_key1" );
			sp->targetname = G_CopyString( "runts_key1" );
			if(key->wait)	sp->wait = key->wait;
			if(key->option) sp->option = key->option;
			if(key->order)  sp->order = key->order;
			ED_CallSpawn( sp );
	...
	while((key = G_Find (key, FOFS(classname), "key_key2")) != NULL)
	{
		...
		{ 
			thugkeytwo++; 
			if(key->target) { level.thugs_key2_target = G_CopyString(key->target); key->target=NULL; }
			sp = G_Spawn();
			sp->classname = G_CopyString( "target_spawner" ); 
			//VectorSet( sp->s.origin, 580, -1830, -170 );
			VectorCopy( key->s.origin, sp->s.origin );
			sp->s.origin[2] += 10;
			sp->style = key->style;
			sp->target = G_CopyString( "key_key2" );
			sp->targetname = G_CopyString( "thugs_key2" );
			if(key->wait)	sp->wait = key->wait;
			if(key->option) sp->option = key->option;
			if(key->order)  sp->order = key->order;
			ED_CallSpawn( sp );
		...
		{ 
			runtkeytwo++; 
			if(key->target) { level.runts_key2_target = G_CopyString(key->target); key->target=NULL; }
			sp = G_Spawn();
			sp->classname = G_CopyString( "target_spawner" ); 
			//VectorSet( sp->s.origin, 580, -1830, -170 );
			VectorCopy( key->s.origin, sp->s.origin );
			...
			sp->style = key->style;
			sp->target = G_CopyString( "key_key2" );
			sp->targetname = G_CopyString( "runts_key2" );
			if(key->wait)	sp->wait = key->wait;
			if(key->option) sp->option = key->option;
			if(key->order)  sp->order = key->order;
			ED_CallSpawn( sp );
	...
	while((key = G_Find (key, FOFS(classname), "key_key3")) != NULL)
	{
		...
		{ 
			thugkeythree++; 
			if(key->target) { level.thugs_key3_target = G_CopyString(key->target); key->target=NULL; }
			...
			sp->s.origin[2] += 10;
			sp->style = key->style;
			sp->target = G_CopyString( "key_key3" );
			sp->targetname = G_CopyString( "thugs_key3" );
			if(key->wait)	sp->wait = key->wait;
			if(key->option) sp->option = key->option;
			if(key->order)  sp->order = key->order;
			ED_CallSpawn( sp );
		...
		{ 
			runtkeythree++; 
			if(key->target) { level.runts_key3_target = G_CopyString(key->target);  key->target=NULL; }
			...
			sp->style = key->style;
			sp->target = G_CopyString( "key_key3" );
			sp->targetname = G_CopyString( "runts_key3" );
			if(key->wait)	sp->wait = key->wait;
			if(key->option) sp->option = key->option;
			if(key->order)  sp->order = key->order;
			ED_CallSpawn( sp );
	...
	level.total_secrets = thugkeyone;
	level.found_secrets = runtkeyone;
	level.total_goals = thugkeytwo;
	level.found_goals = runtkeytwo;
	level.total_monsters = thugkeythree;
	level.killed_monsters = runtkeythree;

	level.unit = level.total_secrets+level.total_goals+level.total_monsters+level.found_secrets+level.found_goals+level.killed_monsters;
...
qboolean SixKeyMap(void)
{
	if(level.unit > 3) return true;
	else return false;
}
...
// parse ents
	while (1)
	{
		...
		{
/*
			///assault2 skip this part cuz i think it removes the rain from assault_jungle
			if (deathmatch->value)
			{
...
//////////////////////////////////////////////////// ASSAULT2 Frank
if (strstr(level.mapname, "assault_jungle"))
	{
		edict_t *sp;

		sp = G_Spawn();
		sp->classname = G_CopyString( "props2_deadgal_headless" ); 
		VectorSet( sp->s.origin, 2200, -1670, -470 );
		sp->s.angles[YAW] = 90;
		sp->s.angles[PITCH] = 0;
		sp->s.angles[ROLL] = 0;
		ED_CallSpawn( sp ); 
		...
		sp = G_Spawn();
		sp->classname = G_CopyString( "props2_deadguy" ); 
		VectorSet( sp->s.origin, 2460, -1240, -632 );
		sp->s.angles[YAW] = 180;
		sp->s.angles[PITCH] = 0;
		sp->s.angles[ROLL] = 0;
		ED_CallSpawn( sp );
		...
		sp = G_Spawn();
		sp->classname = G_CopyString( "smoke_lg" ); 
		VectorSet( sp->s.origin, 2480, -1240, -600 );
		sp->s.angles[YAW] = 180;
		sp->s.angles[PITCH] = 0;
		sp->s.angles[ROLL] = 0;
		ED_CallSpawn( sp );
		...
		sp = G_Spawn();
		sp->classname = G_CopyString( "target_spawner" ); 
		VectorSet( sp->s.origin, 2860, 1292, 270 ); //2760 , 1392, 270
		sp->style = 3;
		sp->speed = 100;
		sp->dmg = 500;
		sp->wait = 1.5;
		sp->target = G_CopyString( "item_watch" );
		sp->targetname = G_CopyString( "drop" );
		sp->s.angles[YAW] = 0;
		sp->s.angles[PITCH] = 5;
		sp->s.angles[ROLL] = 0;
		ED_CallSpawn( sp );
		...
		sp = G_Spawn();
		sp->classname = G_CopyString( "target_spawner" ); 
		VectorSet( sp->s.origin, 2500, 1490, 270 ); //2760 , 1392, 270
		sp->style = 3;
		sp->speed = 100;
		sp->wait = 1.5;
		sp->dmg = 500;
		...
		sp = G_Spawn();
		sp->classname = G_CopyString( "target_spawner" ); 
		VectorSet( sp->s.origin, 2150, 1850, 270 ); //2760 , 1392, 270
		sp->style = 3;
		sp->speed = 100;
		sp->dmg = 500;
		sp->wait = 1.5;
...
/*
		sp = G_Spawn();
		sp->classname = G_CopyString( "target_spawner" ); 
		VectorSet( sp->s.origin, -1990, 1080, -140 );
//		sp->style = 2;
		sp->cast_group = 0; // hate everyone 1=teamfriendly
		sp->skin = 1;	
//		sp->profanity_level = 1;
		sp->onarampage = 1;
		sp->moral = 7;
		sp->target = G_CopyString( "cast_dog" );
		sp->targetname = G_CopyString( "end_game" );
		sp->s.angles[YAW] = 190;
		ED_CallSpawn( sp );

		sp = G_Spawn();
		sp->classname = G_CopyString( "target_spawner" ); 
		VectorSet( sp->s.origin, -2110, 1015, -140 );
		sp->cast_group = 0;
		sp->skin = 2;	
		sp->onarampage = 1;
		sp->moral = 7;
...
/* // virgin20 removes the dogs
	if ( !strcmp( level.mapname, "assault_ty" ) )
	{
			edict_t *sp;

		sp = G_Spawn();
		sp->classname = G_CopyString( "target_spawner" ); 
		VectorSet( sp->s.origin, -2600, 200, 45 );
		sp->style = 1;
		sp->cast_group = 0;
		sp->skin = 2;
		sp->moral = 0;
...
//		sp->profanity_level  = 1;
		...
		sp = G_Spawn();
		sp->classname = G_CopyString( "target_spawner" ); 
		VectorSet( sp->s.origin, -2600, 250, 45 );
		sp->style = 2;
		sp->cast_group = 1;
		sp->skin = 1;
		sp->moral = 0;
...
//		sp->profanity_level = 1;
		...
		sp = G_Spawn();
		sp->classname = G_CopyString( "target_spawner" ); 
		VectorSet( sp->s.origin, -2780, 250, 45 );
		sp->style = 2;
		sp->cast_group = 1;
		sp->skin = 1;
		sp->moral = 0;
...
// cash //assault2 wacked out this folding cash icon
//"xv	350 "
//"cnum "
//"xv	400 "
//"pic 0 " //originally 0
...
// Bagged cash assault2 this is the rotating bagcash icond AND value
"bagcash "
...
void SP_worldspawn (edict_t *ent)
{
	// Let Hogie edit cutscene time assault virgin12
	if (ent->count)	cutscenetime = ent->count;
	else			cutscenetime = 15;
/*
...
// END JOSEPH
	...
	gi.modelindex ("#w_spaz.mdx");				// CLIP_SPAZ virgin16
	gi.modelindex ("#w_pump.mdx");				// CLIP_PUMP virgin17
	gi.modelindex ("#w_ak47.mdx");				// CLIP_TOMMYGUN virgin23
	// ----------------------------------------------------------------------------------
