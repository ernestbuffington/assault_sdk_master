...
// figure momentum add
	...
	////// Assault virgin13 Hogie wants to RJ ////////////////////////////////////////////////////////////////////////
	if(    deathmatch->value
		&& targ==attacker
		&& ((mod==MOD_ROCKET)||(mod==MOD_R_SPLASH))
		&& assault_self_rocket_percent_dmg != 100
		&& assault_self_rocket_percent_dmg > 1
	  )
	{
		float buffer = (float)take;
		float multiplier = (float)(assault_self_rocket_percent_dmg/100);
		buffer *= multiplier;
		take=(int)buffer;
		if(take<1) take=1;
...
edict_t  *SpawnTheWeapon (edict_t *self, char *var_itemname)
{
	...
	else if (!(strcmp (itemname, "weapon_mortar_e")))
	{
		strcpy (itemname, "weapon_mortar");
		e_weapon = true;
		model = gi.modelindex("models/weapons/e_mortar/tris.md2");
	}
