...
int assault_keytargets;
int assault_replaymaps;
int assault_status;
int assault_auto_switch_teams;
int assault_self_rocket_percent_dmg;
// int assault_customweaponfrags;
...
void BanThink(edict_t *ent)
{
	...
	{
		if (ban_source->inuse)
			cprintf( ban_source, PRINT_HIGH, "Vote did not succeed.\n" );
		ban_target = NULL;
		G_FreeEdict(ent);
		return;
	}
...
void Cmd_Response_f (edict_t *ent)
{
	...
	if (!ban_target)
	{
		cprintf( ent, PRINT_HIGH, "Nothing to \"%s\" with.\n", gi.argv(0) );
		return;
	}
	...
	{
		cprintf( ent, PRINT_HIGH, "You've already voted once.\n" );
		return;
	}

	if (!ban_target->inuse || !ban_target->client)
	{
		cprintf( ent, PRINT_HIGH, "The player has already left.\n" );
		ban_target = NULL;
		return;
	}
	...
	{
		cprintf( ent, PRINT_HIGH, "You cannot vote, %s is not on your team.\n", ban_target->client->pers.netname );
		return;
	}
	...
	if (ip && ip[0])
	{
		...
		{
			cprintf( ent, PRINT_HIGH, "You cannot vote.\n" );
			return;
		}
	}
	...
	if (!ip)
	{
		cprintf( ent, PRINT_HIGH, "%s has an unknown IP address\n", ban_target->client->pers.netname );
		ban_target = NULL;
		return;
	}
	if (strcmp(ip, ban_ip))
	{
		cprintf( ent, PRINT_HIGH, "The player has left or been replaced with another player\n" );
		ban_target = NULL;
		return;
	}
	...
	{
		if (ban_source->inuse)
			cprintf( ban_source, PRINT_HIGH, "The vote did not succeed.\n" );

		cprintf(ent, PRINT_HIGH, "Objection noted.\n" );
	...
	{
		cprintf(ent, PRINT_HIGH, "Vote recorded, awaiting votes from other players.\n" );
	}
...
void Vote_Ban( edict_t *ent, char *name )
{
	...
	{
		cprintf( ent, PRINT_HIGH, "You can only VOTE once per 60 seconds\n" );
		return;
	}
	...
	if (last_ban_time)
	{
		...
		{
			cprintf( ent, PRINT_HIGH, "Only one VOTE per 20 seconds allowed\n" );
			return;
		}
	}
	...
	{
		cprintf( ent, PRINT_HIGH, "You cannot kick a member of the other team.\n" );
		return;
	}
	...
	{
		cprintf( ent, PRINT_HIGH, "%s is running the server, cannot BAN host\n", name );
		return;
	}
	...
	cprintf( ent, PRINT_HIGH, "You have voted to kick/ban %s.\nAwaiting responses from other players.\n", ban_target->client->pers.netname );
...
void Cmd_Vote_f (edict_t *ent)
{
	...
	if (!strcmp( cmd, "ban" ))
	{
		...
		if (!name[0])
		{
			cprintf( ent, PRINT_HIGH, "You must supply a name\n" );
			return;
		}
	...
	{
		cprintf( ent, PRINT_HIGH, "Unknown voting command.\n" );
	}
...
/*
	...
	{
		cprintf(ent, PRINT_HIGH, "nav_dynamic 1 must be set to use this command\n");
		return;
	}
*/
	if (!(showpath_on))
...
//	if (!teamplay->value)
//	{
//		cprintf(self, PRINT_HIGH, "This command only available when teamplay is enabled\n");
//		return;
//	}
	...
	self->health = 0;
	meansOfDeath = MOD_RESTART; //assault2
...
*/					}
				...
				if (level.modeset==TEAMPLAY || level.modeset==MATCH
					|| level.modeset==STARTINGPUB || level.modeset==STARTINGMATCH)
				UPDATERATE(self)
//				self->vote = self->client->resp.group; // show class menu
				return;
			}
		}
...
void Cmd_GetTexture_f (edict_t *self)
{
	...
	if (tr.fraction < 1 && tr.surface && tr.surface->name)
	{
		...
		{
			cprintf( self, PRINT_HIGH, "Saving texture: %s\n", tr.surface->name );

			{
				...
				cprintf ( self, PRINT_HIGH, "Saved.\n" );
			}
		}
		else
		{
			cprintf( self, PRINT_HIGH, "Hit texture: %s\n", tr.surface->name );
		}
...
void Cmd_Spawn_f (edict_t *ent)
{
	...
	if(deathmatch->value) return; // assault2
...
void Cmd_ToggleCam_f ( edict_t *ent )
{
	...
	{
		ent->flags += FL_CHASECAM; // virgin16
...
void Cmd_Huds_tgl (edict_t *ent, int direction) //assault2
{
	// adds a limiting timer to prevent overflows
...
//					ent->switch_teams_frame = level.framenum;
	...
	if (!ent->client->pers.inventory[index])
	{
		...
		{
			cprintf (ent, PRINT_HIGH, "You dont have a %s\n", s);
			return;
		}
	}
		...
		while (target = findradius ( target, ent->s.origin, DREWACTIVATEDISTANCE))
		{
			...
			if (target->key > 0)
			{
				switch(target->key)
				{
					case 1:
					{
						if (!ent->client->pers.inventory[ITEM_INDEX(FindItem("Key_One"))])
						{
							gi.sound (ent, CHAN_AUTO, gi.soundindex("world/doors/dr_locked.wav"), 1, ATTN_NORM, 0);
							continue;
						}
					...
					case 2:
					{
						if (!ent->client->pers.inventory[ITEM_INDEX(FindItem("Key_Two"))])
						{
							gi.sound (ent, CHAN_AUTO, gi.soundindex("world/doors/dr_locked.wav"), 1, ATTN_NORM, 0);
							continue;
						}
					...
					case 3:
					{
						if (!ent->client->pers.inventory[ITEM_INDEX(FindItem("Key_Three"))])
						{
							gi.sound (ent, CHAN_AUTO, gi.soundindex("world/doors/dr_locked.wav"), 1, ATTN_NORM, 0);			
							continue;
						}
...
////// assault2 adds a limiting timer to prevent macro chats ////////////////////////////////////////////////////////
...
//		if (!strcmp(ip, last_kick_ip))
//		{
//			cprintf( ent, PRINT_HIGH, "You cannot talk.\n" );
//			return;
//		}
	}
#endif
	...
	if (team)
		Com_sprintf (text, sizeof(text), "<<%s>> ", ent->client->pers.netname);//assault2
	else
		Com_sprintf (text, sizeof(text), "%s: ", ent->client->pers.netname);//assault2
...
void Cmd_CommandList_f (edict_t *ent)// assault2
{
	...
	gi.cprintf(ent, PRINT_HIGH,"POPEYE  <<Popeye's  taunts>>\n");
...
//	ent->client->showscores = COMMANDS; //assault2 fixme
//	DeathmatchScoreboard(ent);
...
void Cmd_MatchScore_f (edict_t *ent)
{
	if (ent->client->pers.admin > NOT_ADMIN ) {
		...
		team_startcash[0]=atoi(gi.argv(1));
		team_startcash[1]=atoi(gi.argv(2));
...
void Cmd_ReportKeys(edict_t *self)
{
	gi.bprintf (PRINT_HIGH,"\nTotal Level Keys: %i\nThugs Keys: %i\nRunts Keys: %i\n\n", level.unit,
		(level.total_secrets + level.total_goals + level.total_monsters) , 
		(level.found_secrets + level.found_goals + level.killed_monsters) );
}
...
void Cmd_SpawnInfo_f(edict_t *self)
{
//	PREVENTSPAM
	gi.bprintf ( PRINT_HIGH, "Acceptable Thug Spawner Style: %i\nAcceptable Runt Spawner Style: %i\n %s.\n", AcceptableThugSpawnerStyle, AcceptableRuntSpawnerStyle, self->client->pers.userinfo);
}


void Cmd_AssaultCmd (edict_t *self, char *cmd, char *number)
{
//	PREVENTSPAM


	if(strstr(cmd,"_keytargets"))
	{
		if(assault_keytargets) 
		{
			assault_keytargets = false;
			gi.bprintf (PRINT_HIGH,"assault_keytargets = false.\n");
		}
		else 
		{
			assault_keytargets = true;
			gi.bprintf (PRINT_HIGH,"assault_keytargets = true.\n");
		}
	}
/*
	else if(strstr(cmd,"_customweaponfrags"))       
	{
		int value = atoi(number);
		if(value < 1 || value > 16) {
			gi.bprintf (PRINT_HIGH,"usage: assault_customweaponfrags <1-16>.\n");
			return;
		}
		assault_customweaponfrags = value;
		gi.bprintf (PRINT_HIGH,"assault_customweaponfrags = %i.\n",assault_customweaponfrags);
	}
*/
	else if(strstr(cmd,"_self_rocket_percent_dmg"))       
	{
		int value = atoi(number);
		if(value < 1 || value > 200) {
			gi.bprintf (PRINT_HIGH,"usage: assault_self_rocket_percent_dmg <1-200>.\n");
			return;
		}
		assault_self_rocket_percent_dmg = value;
		gi.bprintf (PRINT_HIGH,"assault_self_rocket_percent_dmg = %i.\n",assault_self_rocket_percent_dmg);
	}

	else if(strstr(cmd,"_auto_switch_teams"))       
	{
		if(assault_auto_switch_teams) 
		{
			assault_auto_switch_teams = false;
			gi.bprintf (PRINT_HIGH,"assault_auto_switch_teams = false.\n");
		}
		else 
		{
			assault_auto_switch_teams = true;
			gi.bprintf (PRINT_HIGH,"assault_auto_switch_teams = true.\n");
		}
	}

	else if(strstr(cmd,"_replaymaps"))
	{
		if(assault_replaymaps) 
		{
			assault_replaymaps = false;
			gi.bprintf (PRINT_HIGH,"assault_replaymaps = false.\n");
		}
		else
		{
			assault_replaymaps = true;
			gi.bprintf (PRINT_HIGH,"assault_replaymaps = true.\n");
		}
	}

	else if(strstr(cmd,"_status")) Cmd_AssaultStatus();
}

void Cmd_CustomDamage(edict_t *self, char *cmd, char *damage)
{

	int value = atoi(damage);

//	PREVENTSPAM

	if(value < 0 || value > 500 || !value) return;

	if(strstr(cmd,"_pistol"))       CustPistolDmg = value;
	else if(strstr(cmd,"_blakjak")) CustBlakjakDmg = value;
	else if(strstr(cmd,"_shotgun")) CustShotgunDmg = value;
	else if(strstr(cmd,"_tommy"))   CustTommyDmg = value;
	else if(strstr(cmd,"_hmg"))     CustHMGDmg = value;
	else if(strstr(cmd,"_grenade")) CustGrenadeDmg = value;
	else if(strstr(cmd,"_rocket"))  CustRocketDmg = value;
	else if(strstr(cmd,"_flame"))   CustFlameDmg = value;
//	else if(strstr(cmd,"_bomber"))  CustBomberDmg = value;

	gi.bprintf (PRINT_HIGH, "%s = %i.\n", cmd, value);
}

void Cmd_AssaultStatus()
{
	char *msg;

//	gi.bprintf (PRINT_HIGH,"assault_customweaponfrags = %i\n", assault_customweaponfrags);

	if(assault_auto_switch_teams) msg = "true "; else msg = "false";
	gi.bprintf(PRINT_HIGH,"assault_auto_switch_teams: %s\n",msg);

	if(assault_self_rocket_percent_dmg && assault_self_rocket_percent_dmg != 100)
	gi.bprintf(PRINT_HIGH,"assault_self_rocket_percent_dmg: %i\n",assault_self_rocket_percent_dmg);

//	if(assault_bomberfire) msg = "true "; else msg = "false";
//	gi.bprintf(PRINT_HIGH,"assault_bomberfire: %s\n",msg);

//	if(assault_class_icons) msg = "true "; else msg = "false";
//	gi.bprintf(PRINT_HIGH,"assault_class_icons: %s          ",msg);

//	if(assault_classgunuse) msg = "true "; else msg = "false";
//	gi.bprintf(PRINT_HIGH,"assault_classgunuse: %s\n",msg);

//	if(assault_detect_cheats) msg = "true "; else msg = "false";
//	gi.bprintf(PRINT_HIGH,"assault_detect_cheats: %s        ",msg);

//	if(assault_firebomber) msg = "true "; else msg = "false";
//	gi.bprintf(PRINT_HIGH,"assault_firebomber: %s\n",msg);

//	if(assault_health_icons) msg = "true "; else msg = "false";
//	gi.bprintf(PRINT_HIGH,"assault_health_icons: %s         ",msg);

	if(assault_keytargets) msg = "true "; else msg = "false";
	gi.bprintf(PRINT_HIGH,"assault_keytargets: %s\n",msg);

//	if(assault_medic) msg = "true "; else msg = "false";
//	gi.bprintf(PRINT_HIGH,"assault_medic: %s                ",msg);

	if(assault_replaymaps) msg = "true"; else msg = "false";
	gi.bprintf(PRINT_HIGH,"assault_replaymaps: %s\n",msg);

	if(CustBlakjakDmg && CustBlakjakDmg != 15)
		gi.bprintf (PRINT_HIGH,"Custom Crowbar Damage = %i\n", CustBlakjakDmg);
	if(CustPistolDmg && CustPistolDmg != 15)
		gi.bprintf (PRINT_HIGH,"Custom Pistol Damage = %i\n", CustPistolDmg);
	if(CustShotgunDmg && CustShotgunDmg != 8)
		gi.bprintf (PRINT_HIGH,"Custom Shotgun Damage = %i\n", CustShotgunDmg);
	if(CustTommyDmg && CustTommyDmg != 14)
		gi.bprintf (PRINT_HIGH,"Custom Tommy Damage = %i\n", CustTommyDmg);
	if(CustHMGDmg && CustHMGDmg != 50)
		gi.bprintf (PRINT_HIGH,"Custom H.M.G. Damage = %i\n", CustHMGDmg);
	if(CustGrenadeDmg && CustGrenadeDmg != 180)
		gi.bprintf (PRINT_HIGH,"Custom Grenade Damage = %i\n", CustGrenadeDmg);
	if(CustRocketDmg && CustRocketDmg != 125)
		gi.bprintf (PRINT_HIGH,"Custom Rocket Damage = %i\n", CustRocketDmg);
	if(CustFlameDmg && CustFlameDmg != 2)
		gi.bprintf (PRINT_HIGH,"Custom Flame Damage = %i\n", CustFlameDmg);
//	if(CustBomberDmg && CustBomberDmg != 400)
//		gi.bprintf (PRINT_HIGH,"Custom Bomber Damage = %i\n", CustBomberDmg);
}


/////end   Assault2 Commands
/*
=================
ClientCommand
=================
*/
extern void Start_Pub ();
...
#endif
				KICKENT(ent,"%s is being kicked for using a see-thru script!\n");
#ifdef DOUBLECHECK
			} else {
...
//		Cmd_Join_f (ent, gi.argv (1));
	else if (Q_stricmp (cmd, "join") == 0) Cmd_Join_f (ent, gi.argv (1)); //assault2
...
///////////////////////////////////////////////////////////////// assault2 commands
	else if (Q_stricmp (cmd, "skipit") == 0)		Start_Pub();
	else if (!Q_stricmp (cmd, "curse"))				Cmd_Curse_f (ent);
	else if (!Q_stricmp (cmd, "hydro1curse"))		Cmd_Hydro_f (ent);
...
//	else if (!Q_stricmp (cmd, "rummy"))				Cmd_Hogie_f (ent);
	...
	else if (!Q_stricmp (cmd, "barcode"))			Cmd_Barcode_f (ent);
	...
	else if (!Q_stricmp (cmd, "keys"))				Cmd_ReportKeys(ent);
	else if (!Q_stricmp (cmd, "team"))				Cmd_SwitchTeams_f (ent, gi.argv (1));
	else if (!Q_stricmp (cmd, "spawners"))		    Cmd_SpawnInfo_f(ent);
	else if (strstr (cmd, "custdmg_"))			Cmd_CustomDamage (ent, cmd, gi.argv(1));
	else if (strstr (cmd, "assault_"))			Cmd_AssaultCmd (ent, cmd, gi.argv(1));
