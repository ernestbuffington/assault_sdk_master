...
void BecomeExplosion1 (edict_t *self)
{
	...
	else
	{
		...
		if(strstr(self->classname,"key_key")) T_RadiusDamage (self, self, 80, NULL, 350, MOD_EXPLODINGKEY); //assault2
		G_FreeEdict (self);
	}


// Assault2 trigger the level to check for missing keys right away.
	level.power_cubes = 86;
...
void path_corner_cast_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (self->scriptname)
	{
		...
		if(other->option > 1) self->wait = other->option+2; // assault virgin11beta
	}


	if(self->spawnflags & 2) other->cast_info.aiflags |= AI_FASTWALK;
	if(self->spawnflags & 4) other->cast_info.aiflags &= ~AI_FASTWALK;
	if(self->spawnflags & 8) other->cast_info.aiflags |=  AI_RUN_LIKE_HELL;
	if(self->spawnflags & 16) other->cast_info.aiflags &= ~ AI_RUN_LIKE_HELL;
//end assault virgin11beta
	...
	{
//		EP_EventScript( other, self->scriptname );
//		if(other->option > 1) self->wait = other->option+1; // assault virgin11beta
	}
